    ...
    function compareVendorsByStall(a, b) {
      ...
      if (!aText && bText) return 1;
      return normalizeVendorSortText(a.fullName).localeCompare(normalizeVendorSortText(b.fullName), 'tr', { sensitivity: 'base' });
    }
    ...
    function compareAttendanceRowsByStall(a, b) {
      ...
      if (aText && !bText) return -1;
      if (!aText && bText) return 1;
      return normalizeVendorSortText(a.vendorName).localeCompare(normalizeVendorSortText(b.vendorName), 'tr', { sensitivity: 'base' });
    }
    ...
    function getFilteredAttendanceRows() {
      ...
      var colorFilter = document.getElementById('attendanceColorFilter').value;
      ...
      return attendanceSheet.filter(function(item) {
        ...
        var matchesColor = colorFilter === 'all' || (item.stallColor || '') === colorFilter;
        var matchesStatus = statusFilter === 'all' || (statusFilter === 'empty' ? !selected : selected === statusFilter);
        var searchText = [item.vendorName, item.marketName, item.sectionType, item.stallColor, item.stallLabel, item.stallNo, item.note].join(' ').toLocaleLowerCase('tr-TR');
        var matchesSearch = !search || searchText.indexOf(search) !== -1;
        return matchesSection && matchesColor && matchesStatus && matchesSearch;
      }).sort(compareAttendanceRowsByStall);
    }

    function renderStats() {
      var activeMarkets = markets.filter(function(item) { return item.isActive; }).length;
      var actualVendors = vendors.filter(function(item) { return !item.isUnassigned; });
      var totalVendors = actualVendors.length;
      var missingDocs = actualVendors.filter(function(item) { return !item.documents.isComplete; }).length;
      var onLeave = attendanceSheet.filter(function(item) { return !item.isUnassigned && (item.recommendedStatus === 'İzinli' || item.recommendedStatus === 'Raporlu'); }).length;
    ...
    function getFilteredVendors() {
      ...
      var color = document.getElementById('vendorColorFilter').value;
      ...
      return vendors.filter(function(item) {
        var text = [item.fullName, item.identityNumber, item.phone, item.address, item.stallLabel, item.marketName, item.sectionType, item.stallColor].join(' ').toLocaleLowerCase('tr-TR');
        ...
        return matchesMarket && matchesSection && matchesColor && matchesStatus && matchesDocs && matchesSearch;
    ...
    function renderVendorTable() {
      ...
      for (var i = 0; i < rows.length; i++) {
        var item = rows[i];
        var isUnassigned = !!item.isUnassigned;
        var docChips = isUnassigned ? '<span class="doc-chip">Yer kaydı</span>' : item.documents.requiredDocs.map(function(label) {
        ...
        var firstCol = isUnassigned
          ? '<div class="cell-title">' + escapeHtml(item.fullName) + '</div><div class="cell-sub">Bu yer için henüz satıcı tahsisi yapılmadı.</div><div class="cell-sub">Kayıt türü: Boş Yer</div>'
          : '<div class="cell-title">' + escapeHtml(item.fullName) + '</div><div class="cell-sub">TC: ' + escapeHtml(item.identityNumber || 'Girilmeyen') + '</div><div class="cell-sub">Tel: ' + escapeHtml(item.phone || 'Girilmeyen') + '</div><div class="cell-sub">Durum: ' + (item.isActive ? 'Aktif' : 'Pasif') + '</div>';
        var documentCol = isUnassigned
          ? '<div class="cell-title">Yer Tahsisi Yapılmamış</div><div class="doc-list">' + docChips + '</div>'
          : '<div class="cell-title">' + escapeHtml(item.documents.completedCount) + ' / ' + escapeHtml(item.documents.totalRequired) + ' tamam</div><div class="doc-list">' + docChips + '</div>';
        var noteCol = isUnassigned
          ? '<div class="stack"><span class="muted">Boş yer kaydı</span><div class="cell-sub">' + escapeHtml(item.note || 'Not girilmedi') + '</div></div>'
          : '<div class="stack">' +
          ...
          '</div>';
        var actionCol = isUnassigned
          ? '<div class="toolbar">' +
            '<button class="mini-btn primary" type="button" onclick="openVendorModal(' + item.id + ')">Düzenle</button>' +
            '<button class="mini-btn" type="button" disabled>Yoklamada Pasif</button>' +
            '<button class="mini-btn" type="button" disabled>İzin / Rapor Yok</button>' +
            '<button class="mini-btn warn" type="button" onclick="deleteVendor(' + item.id + ')">Sil</button>' +
          '</div>'
          : '<div class="toolbar">' +
            '<button class="mini-btn primary" type="button" onclick="openVendorModal(' + item.id + ')">Düzenle</button>' +
    ...
    function getFilteredLeaves() {
      ...
      var search = document.getElementById('leaveSearchInput').value.trim().toLocaleLowerCase('tr-TR');
      var today = new Date().toISOString().slice(0, 10);
      return leaveRecords.filter(function(item) {
        var matchesMarket = marketId === 'all' || String(item.marketId) === String(marketId);
        var matchesType = type === 'all' || item.leaveType === type;
        var matchesMode = true;
        if (activeMode === 'open') matchesMode = item.endDate >= today;
        else if (activeMode === 'active') matchesMode = item.startDate <= today && item.endDate >= today;
        else if (activeMode === 'expired') matchesMode = item.endDate < today;
        var haystack = [item.vendorName, item.marketName, item.sectionType, item.stallLabel, item.leaveType, item.note, item.startDateText, item.endDateText, item.remainingText].join(' ').toLocaleLowerCase('tr-TR');
        var matchesSearch = !search || haystack.indexOf(search) !== -1;
        return matchesMarket && matchesType && matchesMode && matchesSearch;
      });
    }


    function buildLeaveExportQuery() {
      var query = [];
      var marketId = document.getElementById('leaveMarketFilter').value;
      var leaveType = document.getElementById('leaveTypeFilter').value;
      var activeOnly = document.getElementById('leaveActiveFilter').value;
      var search = document.getElementById('leaveSearchInput').value.trim();
      if (marketId) query.push('marketId=' + encodeURIComponent(marketId));
      if (leaveType) query.push('leaveType=' + encodeURIComponent(leaveType));
      if (activeOnly) query.push('activeOnly=' + encodeURIComponent(activeOnly));
      if (search) query.push('search=' + encodeURIComponent(search));
      return query.join('&');
    }

    function downloadLeaveExportExcel() {
      var query = buildLeaveExportQuery();
      window.location.href = '/api/markets/leave-records/export.xlsx' + (query ? ('?' + query) : '');
    }

    function printLeaveReport() {
      var rows = getFilteredLeaves();
      if (!rows.length) {
        alert('PDF oluşturmak için listede en az bir izin / rapor kaydı olmalıdır.');
        return;
      }
      var marketText = document.getElementById('leaveMarketFilter').selectedOptions[0] ? document.getElementById('leaveMarketFilter').selectedOptions[0].textContent : 'Tüm Pazarlar';
      var typeText = document.getElementById('leaveTypeFilter').selectedOptions[0] ? document.getElementById('leaveTypeFilter').selectedOptions[0].textContent : 'Tüm Türler';
      var activeText = document.getElementById('leaveActiveFilter').selectedOptions[0] ? document.getElementById('leaveActiveFilter').selectedOptions[0].textContent : 'Açık Kayıtlar';
      var searchText = document.getElementById('leaveSearchInput').value.trim() || 'Arama yok';
      var nowText = new Date().toLocaleString('tr-TR');
      var summaryHtml = [
        '<span class="doc-chip">Toplam: ' + rows.length + '</span>',
        '<span class="doc-chip ok">İzinli: ' + rows.filter(function(item){ return item.leaveType === 'İzinli'; }).length + '</span>',
        '<span class="doc-chip warn">Raporlu: ' + rows.filter(function(item){ return item.leaveType === 'Raporlu'; }).length + '</span>'
      ].join('');
      var tableRows = rows.map(function(item) {
        return '<tr>' +
          '<td><strong>' + escapeHtml(item.vendorName || '-') + '</strong><div class="sub">Eklenme: ' + escapeHtml(item.createdAt || '-') + '</div></td>' +
    ...
    function renderLeaveTable() {
      ...
      for (var i = 0; i < rows.length; i++) {
        ...
        html += '<tr>' +
          ...
          '<td><div class="cell-title">' + escapeHtml(item.marketName) + '</div><div class="cell-sub">' +
            '<span style="display:inline-block;margin-right:14px;">' + escapeHtml(item.sectionType || '-') + '</span>' +
            '<span style="display:inline-block;">Yer: ' + escapeHtml(item.stallLabel || 'Atanmadı') + '</span>' +
          '</div></td>' +
          '<td>' + (item.leaveType === 'Raporlu' ? '<span class="badge warn">Raporlu</span>' : '<span class="badge info">İzinli</span>') + '</td>' +
          '<td><div class="cell-title">' + escapeHtml(item.startDateText) + ' - ' + escapeHtml(item.endDateText) + '</div><div class="cell-sub">' + escapeHtml(item.remainingText || '-') + '</div></td>' +
          '<td>' + escapeHtml(item.note || 'Açıklama girilmedi') + '</td>' +
    ...
    function renderAttendanceSheet() {
      ...
      if (infoBox) {
        var assignedCount = attendanceSheet.filter(function(item) { return !item.isUnassigned; }).length;
        var unassignedCount = attendanceSheet.filter(function(item) { return item.isUnassigned; }).length;
        infoBox.textContent = 'Toplam ' + assignedCount + ' satıcı ve ' + unassignedCount + ' boş yer var. Filtreye göre ' + rows.length + ' satır gösteriliyor. Liste bölüm, renk ve yer numarasına göre sıralanır. Üreticide önce Yeşil, sonra Kırmızı gelir.';
      }
      if (!rows.length) {
      ...
      for (var i = 0; i < rows.length; i++) {
        ...
        var autoInfo = item.isUnassigned
          ? '<span class="badge gray">Yer Tahsisi Yapılmamış</span><div class="cell-sub">Yoklama listesinde görünür, işlem yapılamaz.</div>'
          : (item.leaveType
        ...
        html += '<tr>' +
          ...
          '</div></td>' +
          '<td><input class="attendance-note" type="text" value="' + escapeHtml(item.note || '') + '" ' + (item.isLocked ? 'disabled' : '') + ' oninput="updateAttendanceNote(' + item.vendorId + ', this.value)" placeholder="Kısa not" /></td>' +
        '</tr>';
      }
      body.innerHTML = html;
    ...
    function clearVendorFilters() {
      ...
      document.getElementById('vendorColorFilter').value = 'all';
    ...
    function printVendorReport() {
      ...
      var colorText = document.getElementById('vendorColorFilter').selectedOptions[0] ? document.getElementById('vendorColorFilter').selectedOptions[0].textContent : 'Tüm Renkler';
      ...
      var nowText = new Date().toLocaleString('tr-TR');
      var summaryHtml = [
        '<span class="doc-chip">Toplam Satıcı: ' + rows.filter(function(item){ return !item.isUnassigned; }).length + '</span>',
        '<span class="doc-chip">Boş Yer: ' + rows.filter(function(item){ return item.isUnassigned; }).length + '</span>',
        '<span class="doc-chip ok">Aktif: ' + rows.filter(function(item){ return !item.isUnassigned && item.isActive; }).length + '</span>',
        '<span class="doc-chip missing">Pasif: ' + rows.filter(function(item){ return !item.isUnassigned && !item.isActive; }).length + '</span>',
        '<span class="doc-chip">Belgesi Tam: ' + rows.filter(function(item){ return !item.isUnassigned && item.documents && item.documents.isComplete; }).length + '</span>',
        '<span class="doc-chip">Belgesi Eksik: ' + rows.filter(function(item){ return !item.isUnassigned && item.documents && !item.documents.isComplete; }).length + '</span>'
      ].join('');
      var tableRows = rows.map(function(item) {
        ...
        return '<tr>' +
          '<td><strong>' + escapeHtml(item.fullName) + '</strong>' + (item.isUnassigned ? '<div class="sub">Bu yer için satıcı tahsisi yapılmamıştır.</div>' : '<div class="sub">TC: ' + escapeHtml(item.identityNumber || '-') + '</div><div class="sub">Tel: ' + escapeHtml(item.phone || '-') + '</div>') + '</td>' +
          '<td>' + escapeHtml(item.marketName || '-') + '<div class="sub">' + escapeHtml(item.scheduledDayLabel || '') + '</div></td>' +
          '<td>' + escapeHtml(item.sectionType || '-') + '<div class="sub">Yer: ' + escapeHtml(item.stallLabel || '-') + '</div></td>' +
          '<td>' + (item.isUnassigned ? 'Boş Yer' : (item.isActive ? 'Aktif' : 'Pasif')) + '</td>' +
          '<td>' + (item.isUnassigned ? 'Yer Tahsisi Yapılmamış' : escapeHtml((item.documents.completedCount || 0) + ' / ' + (item.documents.totalRequired || 0) + ' tamam')) + '<div class="sub">' + escapeHtml(item.isUnassigned ? '-' : (docsText || '-')) + '</div></td>' +
      ...
      var reportHtml = '' +
        ...
        '<div class="meta-grid">' +
          ...
          '<div class="meta-card"><div class="meta-label">Renk</div><div class="meta-value">' + escapeHtml(colorText) + '</div></div>' +
    ...
    function clearLeaveFilters() {
      ...
      document.getElementById('leaveActiveFilter').value = 'open';
    ...
    var activeMarketModuleTab = 'vendors';

    function setActiveMarketModuleTab(tabKey, options) {
      var settings = options || {};
      activeMarketModuleTab = tabKey;
      var buttons = document.querySelectorAll('[data-module-tab]');
      var panels = document.querySelectorAll('[data-module-panel]');
      buttons.forEach(function(button) {
        var isActive = button.getAttribute('data-module-tab') === tabKey;
        button.classList.toggle('active', isActive);
        button.setAttribute('aria-pressed', isActive ? 'true' : 'false');
      });
      panels.forEach(function(panel) {
        var isActive = panel.getAttribute('data-module-panel') === tabKey;
        panel.classList.toggle('active', isActive);
      });
      if (settings.scroll !== false) {
        var targetPanel = document.querySelector('[data-module-panel="' + tabKey + '"]');
        if (targetPanel) {
    ...
    function scrollToAttendance() {
      setActiveMarketModuleTab('attendance');
    }
    ...
    function printAttendanceDetailReport() {
      ...
      var tableRows = rows.map(function(item, index) {
        return '<tr>' +
          ...
          '<td>' + escapeHtml(item.isUnassigned ? '-' : (item.status || '-')) + '</td>' +
          '<td>' + escapeHtml(item.note || '-') + '</td>' +
          '<td>' + escapeHtml(item.updatedAt || '-') + '</td>' +
        '</tr>';
      }).join('');
    ...
    document.addEventListener('DOMContentLoaded', async function() {
      ...
      setActiveMarketModuleTab('vendors', { scroll: false });
      try {
        await loadMarkets();
