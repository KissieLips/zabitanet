    ...
    function matchBadge(item) {
      if (item.businessId) return '<span class="badge active">Firma Bağlandı</span>';
      if (item.matchStatus === 'Onay Bekliyor' && item.suggestedBusinessId) return '<span class="badge application">Eşleşme Onayı Bekliyor</span>';
      if (item.canCreateBusinessSuggestion) return '<span class="badge info">Firma Kaydı Önerilir</span>';
      return '<span class="badge gray">Firma Bağlantısı Yok</span>';
    }
    ...
    function setLinkedBusinessInfo(item) {
      ...
      if (!item) {
        box.textContent = 'Bu ruhsat kaydı firmadan bağımsız tutulur. Sistem sadece eşleşme adayı önerir; bağlama onayla yapılır.';
        return;
      }
      if (item.businessId) {
      ...
      if (item.suggestedBusinessId) {
        box.innerHTML = 'Eşleşme adayı: <strong>' + escapeHtml(item.suggestedBusinessName || ('Firma #' + item.suggestedBusinessId)) + '</strong> · Puan: <strong>' + escapeHtml(item.matchScore || 0) + '</strong>';
        return;
      }
      if (item.canCreateBusinessSuggestion) {
        box.innerHTML = 'Firma önerisi hazır: <strong>Bu ruhsattan yeni firma kaydı oluşturulabilir.</strong>';
        return;
      }
      box.textContent = 'Bu ruhsat kaydı firmadan bağımsız tutulur. Şu an onay bekleyen bir eşleşme adayı bulunmuyor.';
    }
    ...
    function renderLicenseDetail(item) {
      ...
      var matchActionHtml = '<span class="muted">Şu an onay bekleyen eşleşme adayı yok</span>';
      if (item.businessId) {
        matchActionHtml = '<span class="muted">Bu ruhsat kaydı firmaya bağlandı.</span>';
      } else if (item.suggestedBusinessId) {
        matchActionHtml = '<div class="action-row" style="margin-top:6px;">' +
          '<button class="mini-btn primary" type="button" onclick="approveMatch(' + item.id + ')">Eşleşmeyi Onayla</button>' +
          '<button class="mini-btn" type="button" onclick="clearMatch(' + item.id + ')">Adayı Temizle</button>' +
        '</div>';
      }

      var createBusinessHtml = '<span class="muted">Bu ruhsat için firma önerisi yok.</span>';
      if (item.businessId) {
        createBusinessHtml = '<span class="muted">Firma kaydı zaten oluşturuldu.</span>';
      } else if (item.canCreateBusinessSuggestion) {
        createBusinessHtml = '<div class="suggestion-box">' +
          '<div class="suggestion-box-title">Firma Kaydı Önerisi</div>' +
          '<div class="suggestion-box-text">' + escapeHtml(item.createBusinessSuggestionNote || 'Bu aktif ruhsattan yeni firma kaydı önerilebilir.') + ' Eksik alanları firma detayında sonra tamamlayabilirsin.</div>' +
          '<div class="action-row"><button class="mini-btn primary" type="button" onclick="createBusinessFromLicense(' + item.id + ')">Firmayı Oluştur</button><button class="mini-btn" type="button" onclick="openLicenseModal(' + item.id + ')">Ruhsatı Düzenle</button></div>' +
        '</div>';
      }
      ...
      var html = '' +
        '<div class="detail-card">' +
          ...
          '<div class="detail-list">' +
            ...
            '<div class="detail-item"><div class="detail-label">Bağlantı Durumu</div><div class="detail-value">' + matchBadge(item) + '</div></div>' +
            ...
            '<div class="detail-item"><div class="detail-label">Önerilen Firma</div><div class="detail-value">' + (item.suggestedBusinessName ? (escapeHtml(item.suggestedBusinessName) + ' <span class="muted">(puan: ' + escapeHtml(item.matchScore || 0) + ')</span><br><span class="muted">' + escapeHtml(item.matchNote || '') + '</span>') : '<span class="muted">Öneri yok</span>') + '</div></div>' +
            '<div class="detail-item"><div class="detail-label">Firma Ekranı</div><div class="detail-value">' + businessHtml + '</div></div>' +
            '<div class="detail-item"><div class="detail-label">Eşleşme İşlemi</div><div class="detail-value">' + matchActionHtml + '</div></div>' +
            '<div class="detail-item"><div class="detail-label">Firma Kaydı Önerisi</div><div class="detail-value">' + createBusinessHtml + '</div></div>' +
          '</div>' +
        '</div>' +
        '<div class="detail-card">' +
    ...
    function renderLicenseTable() {
      ...
      for (var i = 0; i < rows.length; i++) {
        ...
        var businessLink = item.businessId
          ...
          : (item.suggestedBusinessId
              ...
              : (item.canCreateBusinessSuggestion
                  ? '<div class="cell-sub">Firma önerisi hazır · Manuel onay bekliyor</div>'
        ...
        var actionButtons = '';
        if (!item.businessId && item.suggestedBusinessId) {
          actionButtons += '<button class="mini-btn primary" type="button" onclick="approveMatch(' + item.id + ')">Eşleşmeyi Onayla</button>';
          actionButtons += '<button class="mini-btn" type="button" onclick="clearMatch(' + item.id + ')">Adayı Temizle</button>';
        }
        if (!item.businessId && item.canCreateBusinessSuggestion) {
          actionButtons += '<button class="mini-btn primary" type="button" onclick="createBusinessFromLicense(' + item.id + ')">Firmayı Oluştur</button>';
        }
        actionButtons += '<button class="mini-btn" type="button" onclick="openLicenseDetail(' + item.id + ')">Ruhsat Detay</button>';
        ...
        html += '<tr>' +
          '<td><div class="stack"><div class="cell-title">' + escapeHtml(item.tradeName || '-') + '</div><div class="cell-sub">' + escapeHtml(item.ownerName || 'Sahip bilgisi yok') + '</div><div class="cell-sub">' + matchBadge(item) + '</div>' + businessLink + '</div></td>' +
    ...
    async function createBusinessFromLicense(id) {
      if (!confirm('Bu ruhsattan yeni firma kaydı oluşturulsun mu? Eksik bilgiler firma detayında daha sonra tamamlanabilir.')) return;
      try {
        var response = await fetch('/api/licenses/' + id + '/create-business', { method: 'POST' });
        var data = await response.json();
        if (!response.ok) throw new Error(data.error || 'Firma kaydı oluşturulamadı.');
        await loadBusinesses();
        await loadLicenses();
        closeDetailModal();
        showToast('Ruhsattan yeni firma kaydı oluşturuldu.');
      } catch (error) {
        alert(error.message || 'Firma kaydı oluşturulamadı.');
      }
    }

    async function approveMatch(id) {
      if (!confirm('Önerilen firma ile bu ruhsat kaydı bağlansın mı?')) return;
      try {
        ...
        await loadLicenses();
        closeDetailModal();
        showToast('Eşleşme onaylandı ve ruhsat firmaya bağlandı.');
      } catch (error) {
        alert(error.message || 'Eşleşme onaylanamadı.');
      }
    }

    async function clearMatch(id) {
